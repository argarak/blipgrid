...
document.addEventListener("DOMContentLoaded", e => {
  ...
  let ampKnobsContainer = document.getElementById("ampKnobs");
  let ampKnobs = ampKnobsContainer.querySelectorAll("ui-knob");

  let freqKnobsContainer = document.getElementById("freqKnobs");
  let freqKnobs = freqKnobsContainer.querySelectorAll("ui-knob");

  let noiseKnobsContainer = document.getElementById("noiseKnobs");
  let noiseKnobs = noiseKnobsContainer.querySelectorAll("ui-knob");

  let knobOffset = 100;

  for (let knob of ampKnobs) {
    knob.addEventListener("input", e => {
      let type = e.target.getAttribute("label");

      switch (type) {
        case "attack":
          env.attack = e.target.pos / knobOffset + 0.01;
          break;
        case "decay":
          env.decay = e.target.pos / knobOffset;
          break;
        case "sustain":
          env.sustain = e.target.pos / knobOffset;
          break;
        case "release":
          env.release = e.target.pos / knobOffset;
          break;
      }
    });
  }

  for (let knob of noiseKnobs) {
    knob.addEventListener("input", e => {
      let type = e.target.getAttribute("label");

      switch (type) {
        case "decay":
          noiseEnv.decay = e.target.pos / knobOffset;
          break;
        case "cutoff":
          filter.frequency.value = e.target.pos * 100;
          break;
        case "resonance":
          filter.Q.value = e.target.pos / 5;
          break;
      }
    });
  }

  for (let knob of freqKnobs) {
    knob.addEventListener("input", e => {
      let type = e.target.getAttribute("label");

      switch (type) {
        case "attack":
          freqEnv.attack = e.target.pos / knobOffset + 0.01;
          break;
        case "decay":
          freqEnv.decay = e.target.pos / knobOffset;
          break;
        case "sustain":
          freqEnv.sustain = e.target.pos / knobOffset;
          break;
        case "release":
          freqEnv.release = e.target.pos / knobOffset;
          break;
      }
    });
  }
